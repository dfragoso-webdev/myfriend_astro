...
const BranchSelector = ({ isScrolled, label }: any) => {
  ...
  return (
    <div className="relative group">
      ...
      >
        <option value="" disabled>{label ?? "Sucursales"}</option>
        ...
        <div className="p-5 sm:p-6 bg-gray-50 border-t border-gray-100 space-y-4">
          <div className="relative">
            <select
              value={selectedBranch}
              onChange={(e) => setSelectedBranch(e.target.value)}
              className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm sm:text-sm appearance-none outline-none focus:ring-2 focus:ring-secondary/20"
              aria-label="Seleccionar sucursal"
            >
              <option value="" disabled>{branchLabel ?? "Sucursales"}</option>
              <option value="royal">Royal Hideway</option>
            </select>
